...
Una aplicación Flutter que implementa una arquitectura moderna y robusta para el seguimiento de datos deportivos, con autenticación basada en Supabase y control de partidos en tiempo real.

## 🚀 Inicio Rápido - Flujo de Autenticación

Esta primera entrega implementa un flujo completo de autenticación usando las pantallas de **login**, **register** y **profile**.
...
Al ejecutar `flutter run` después de haber instalado las dependencias necesarias, la aplicación mostrará la pantalla de registro. Puede registrar un usuario nuevo o navegar hacia la pantalla de login para usar uno existente.

Si existe algún error durante alguno de estos procesos, la aplicación lo notificará y se quedará en dichas pantallas. Si el proceso es exitoso será redirigido a la pantalla de profile donde podrá ver un resumen de la sesión activa así como información de partidos y equipos situada allí como placeholder para ser implementada de forma real en una posterior entrega.
